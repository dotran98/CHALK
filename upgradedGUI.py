    ...
    def __export(self):
        try:
            ...
            for system in self.data_hub.systemList:
                ...
                pdf.ln(10)
                pdf.cell(20, 0.0)
                pdf.cell(page_width, 0.0, "IP: " + str(system.ipAddress))
                pdf.ln(10)
                pdf.cell(20, 0.0)
                pdf.cell(page_width, 0.0, "Operating System:" + str(system.operatingSystem))
                pdf.ln(10)
                pdf.cell(20, 0.0)
                pdf.cell(page_width, 0.0, "Open Ports and Services: {}".format(system.numOpenPorts))
                pdf.ln(10)
                for port in system.openPorts[0]:
                    ...
                    for details in temp:
                        if len(details) > 20:
                            ...
                            pdf.cell(40, 0.0)
                            pdf.cell(page_width, 0.0, "State: "+details[0].split(':')[1].replace("'", ''))
                            pdf.ln(10)
                            pdf.cell(40, 0.0)
                            pdf.cell(page_width, 0.0, "Product: " + details[3].split(':')[1].replace("'", ''))
                            pdf.ln(10)
                            pdf.cell(40, 0.0)
                            pdf.cell(page_width, 0.0, "Version: " + details[4].split(':')[1].replace("'", ''))
                            pdf.ln(10)
                            pdf.cell(40, 0.0)
                            pdf.cell(page_width, 0.0, "Extra Information: " + details[5].split(':')[1].replace("'", ''))
                            pdf.ln(10)
                            pdf.cell(40, 0.0)
                            pdf.cell(page_width, 0.0, "Configuration: " + details[6].split(':')[1].replace("'", ''))
                            pdf.ln(10)
                            pdf.cell(40, 0.0)
                            pdf.cell(page_width, 0.0, "CPE: " + details[7].split(':')[1].replace("'", ''))
                            pdf.ln(10)
                pdf.cell(20, 0.0)
